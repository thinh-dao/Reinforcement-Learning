...
class Linear(DQN):
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        input = torch.flatten(state, start_dim=1, end_dim=3)
        if network == 'q_network': out = self.q_network(input)
        else: out = self.target_network(input)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def update_target(self):
        ...
        torch.save(self.q_network.state_dict(), "model.pt") # saving the model
        self.target_network.load_state_dict(torch.load("model.pt")) # loading the model
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        Q_sa = torch.sum(q_values * F.one_hot(actions.long(), num_actions), 1)
        Q_samp = rewards + (1.0 - done_mask.float()) * gamma * torch.max(target_q_values, 1)[0]
        return F.mse_loss(Q_sa, Q_samp)
        ##############################################################
        ######################## END YOUR CODE #######################
